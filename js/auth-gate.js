...
(async () => {
  ...
  gate.innerHTML = `
    <div class="sn-auth-gate-box">
      ...
      <div class="sn-auth-gate-btns">
        <a id="gate-reg-link" href="${isEn ? "/en/pages/register.html" : "/de/pages/register.html"}" class="sn-auth-gate-btn primary">${isEn ? "Register for free" : "Kostenlos registrieren"}</a>
        <a id="gate-login-link" href="${isEn ? "/en/pages/login.html" : "/de/pages/login.html"}" class="sn-auth-gate-secondary">${isEn ? "Already have an account? Log in" : "Bereits registriert? Einloggen"}</a>
      </div>
    </div>`;
  ...
  // Explizite Navigation erzwingen, falls die Browser-Standardaktion im Modal blockiert wird
  document.getElementById("gate-reg-link").onclick = (e) => {
    e.preventDefault();
    window.location.href = e.currentTarget.href;
  };
  document.getElementById("gate-login-link").onclick = (e) => {
    e.preventDefault();
    window.location.href = e.currentTarget.href;
  };
