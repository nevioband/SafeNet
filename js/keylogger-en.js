(!(function () {
  ...
  (e.addEventListener("keydown", (e) => {
    ...
    (" " === e.key
      ...
      (n.textContent = l.length + " keys recorded"));
  }),
    o.addEventListener("click", () => {
      ((l.length = 0),
        (t.textContent = "No keystrokes recorded yet."),
        (n.textContent = ""),
        (e.value = ""),
        e.focus());
    }));
  ...
  (function () {
    ...
    function s(e, t) {
      ...
      if (null === c) return;
      const s = new Date().toLocaleTimeString("en", { hour12: !1 }),
        u = "user" === e ? "Username" : "Password";
      (0 === d.length && (n.textContent = ""),
        d.push("[" + s + "] " + u + " → " + c),
        (n.textContent = d.join("\n")),
        (n.scrollTop = n.scrollHeight),
        (l.textContent = d.length + " events transmitted"),
        (o.style.opacity = "0.15"),
        setTimeout(() => {
          o.style.opacity = "1";
        }, 150));
    }
    (e.addEventListener("keydown", (e) => s("user", e.key)),
      t.addEventListener("keydown", (e) => s("pass", e.key)),
      c.addEventListener("click", () => {
        ((d.length = 0),
          (n.textContent = "Waiting for input…"),
          (l.textContent = ""),
          (e.value = ""),
          (t.value = ""));
      }));
