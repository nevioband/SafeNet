...
function metaRow(e) {
  return `<div class="np-meta">\n    <time class="np-date">${escHtml(formatDate(e.date))}</time>\n    <a href="${safeUrl(e.link)}" class="np-readmore" target="_blank" rel="noopener noreferrer">Read more →</a>\n  </div>`;
}
...
async function loadNews() {
  ...
  try {
    ...
    const t = n.items ?? [];
    if (!t.length) throw new Error("No news received");
    const [s, ...c] = t,
      a = c.slice(0, 2),
      l = c.slice(2);
    ...
    e.innerHTML = `<p class="np-error">Error loading news: ${escHtml(r.message)}</p>`;
  }
}
loadNews();
