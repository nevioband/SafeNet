...
(document.getElementById("avatarCircle").addEventListener("click", () => {
  ...
  (async () => {
    let t,
      ...
      r = null,
      s = "";
    const { data: o } = await e.auth.getSession();
    if (((t = o?.session), !t))
      return void (window.location.href = "/de/pages/login.html");
    if (
      ((n = t.user.id),
      (a = t.user.email),
      (d = a.charAt(0).toUpperCase()),
      (s = t.user.user_metadata?.display_name || ""),
      (document.getElementById("profileEmail").textContent = a),
      (document.getElementById("avatarInitial").textContent = s
        ? s.charAt(0).toUpperCase()
        : d),
      (document.getElementById("displayName").value = s),
      s)
    ) {
      const e = document.getElementById("profileDisplayName");
      ((e.textContent = s), (e.style.display = "block"));
    }
    const i = t.user.user_metadata?.avatar_b64;
    async function l() {
      const { data: t, error: n } = await e.auth.mfa.listFactors();
      if (n)
        return void c("tfaMsg", "Fehler beim Laden des 2FA-Status.", "error");
    ...
    (i &&
      (document.getElementById("avatarCircle").innerHTML =
        `<img src="${i}" alt="${d}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`),
      document
        .getElementById("avatarInput")
        .addEventListener("change", async (t) => {
          const n = t.target.files[0];
          if (!n) return;
          c("avatarMsg", "Wird verarbeitet…", "info");
          const a = await (function (e, t) {
                ...
                ((a.onload = (e) => {
                  ...
                  ((a.onload = () => {
                    let e = a.width,
                      d = a.height;
                    e > d && e > t
                      ? ((d = Math.round((d * t) / e)), (e = t))
                      : d >= e &&
                        d > t &&
                        ((e = Math.round((e * t) / d)), (d = t));
                    const r = document.createElement("canvas");
                    ((r.width = e),
                      (r.height = d),
                      r.getContext("2d").drawImage(a, 0, 0, e, d),
                      n(r.toDataURL("image/jpeg", 0.85)));
                  }),
            ...
            { error: r } = await e.auth.updateUser({ data: { avatar_b64: a } });
          r
            ? c("avatarMsg", "Fehler: " + r.message, "error")
            : ((document.getElementById("avatarCircle").innerHTML =
                `<img src="${a}" alt="${d}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`),
              c(
                "avatarMsg",
      ...
      document
        ...
        .addEventListener("click", async () => {
          ...
          ((t.textContent = "Wird vorbereitet…"), (t.disabled = !0));
          const { data: n, error: a } = await e.auth.mfa.enroll({
            factorType: "totp",
            friendlyName: "SafeNet Authenticator",
          });
          ((t.textContent = "2FA aktivieren"),
            (t.disabled = !1),
            a
              ? c("tfaMsg", "Fehler: " + a.message, "error")
              : ((r = n.id),
      ...
      document
        ...
        .addEventListener("click", async () => {
          ...
          if (!t || 6 !== t.length)
            return void c(
              "tfaMsg",
              "Bitte einen 6-stelligen Code eingeben.",
              "error",
            );
          const n = document.getElementById("verifyBtn");
          ((n.textContent = "Wird überprüft…"), (n.disabled = !0));
          const { data: a, error: d } = await e.auth.mfa.challenge({
            factorId: r,
          });
          if (d)
            return (
              c("tfaMsg", "Fehler: " + d.message, "error"),
              (n.textContent = "Bestätigen & aktivieren"),
              void (n.disabled = !1)
            );
          const { error: s } = await e.auth.mfa.verify({
            factorId: r,
            challengeId: a.id,
            code: t,
          });
          ((n.textContent = "Bestätigen & aktivieren"),
            (n.disabled = !1),
            s
              ? c("tfaMsg", "Falscher Code. Bitte versuche es erneut.", "error")
              : (c("tfaMsg", "2FA wurde erfolgreich aktiviert!", "success"),
      ...
      document
        ...
        .addEventListener("click", async () => {
          (r && (await e.auth.mfa.unenroll({ factorId: r }), (r = null)),
            await l());
        }),
      document
        ...
        .addEventListener("click", async () => {
          if (
            !(await window.snConfirm(
              "2FA wirklich deaktivieren? Dein Konto wird dadurch weniger sicher.",
            ))
          )
            return;
          c("tfaMsg", "", "hide");
          const t = document.getElementById("disableBtn");
          ((t.textContent = "Wird deaktiviert…"), (t.disabled = !0));
          const { error: n } = await e.auth.mfa.unenroll({ factorId: r });
          ((t.textContent = "2FA deaktivieren"),
            (t.disabled = !1),
            n
              ? c("tfaMsg", "Fehler: " + n.message, "error")
              : (c("tfaMsg", "2FA wurde deaktiviert.", "success"), await l()));
        }),
      document
        ...
        .addEventListener("click", async () => {
          ...
          ((n.textContent = "Wird gespeichert…"), (n.disabled = !0));
          const { error: a } = await e.auth.updateUser({
            data: { display_name: t },
          });
          if (((n.textContent = "Speichern"), (n.disabled = !1), a))
            return void c("nameMsg", "Fehler: " + a.message, "error");
          (c("nameMsg", "Anzeigename wurde gespeichert.", "success"),
            (document.getElementById("avatarInitial").textContent = t
              ? t.charAt(0).toUpperCase()
              : d));
          const r = document.getElementById("profileDisplayName");
          t
      ...
      document
        ...
        .addEventListener("click", async () => {
          const t = document.getElementById("oldEmail").value.trim(),
            ...
            d = document.getElementById("newEmail").value.trim();
          if (!t || !t.includes("@"))
            return void c(
              "emailMsg",
              "Bitte deine aktuelle E-Mail-Adresse eingeben.",
              "error",
            );
          if (t.toLowerCase() !== a.toLowerCase())
            return void c(
              "emailMsg",
              "Die eingegebene E-Mail-Adresse stimmt nicht mit deiner aktuellen überein.",
              "error",
            );
          if (!n)
            return void c(
          ...
          if (!d || !d.includes("@"))
            return void c(
              "emailMsg",
              "Bitte eine gültige neue E-Mail-Adresse eingeben.",
              "error",
            );
          ...
          const r = document.getElementById("changeEmailBtn");
          ((r.textContent = "Wird überprüft…"), (r.disabled = !0));
          const { error: s } = await e.auth.signInWithPassword({
            email: a,
            password: n,
          });
          if (s)
            return (
              c("emailMsg", "Das aktuelle Passwort ist falsch.", "error"),
              (r.textContent = "Bestätigungsmail senden"),
              void (r.disabled = !1)
            );
          r.textContent = "Wird gesendet…";
          const { error: o } = await e.auth.updateUser({ email: d });
          ((r.textContent = "Bestätigungsmail senden"),
            (r.disabled = !1),
            o
              ? c("emailMsg", "Fehler: " + o.message, "error")
              : (c(
                  "emailMsg",
                  `Bestätigungsmail wurde an ${d} gesendet. Bitte klicke auf den Link in der Mail.`,
                  "success",
                ),
      ...
      (async () => {
        ...
        if (a)
          ...
          const e =
            '<span title="Tresor öffnen um Statistiken zu laden" style="font-size:11px;opacity:0.5;cursor:help;">?</span>';
      ...
      document
        ...
        .addEventListener("click", async () => {
          if ("LÖSCHEN" !== document.getElementById("deleteConfirmInput").value)
            return void c(
              "deleteMsg",
              'Bitte gib genau "LÖSCHEN" ein.',
              "error",
            );
          const t = document.getElementById("confirmDeleteBtn");
          ((t.textContent = "Wird gelöscht…"), (t.disabled = !0));
          const {
            data: { session: a },
          } = await e.auth.getSession();
          await e.from("passwords").delete().eq("user_id", n);
          const d = await fetch("/api/delete-account", {
            method: "POST",
          ...
          if (!d.ok)
            return (
              c(
                "deleteMsg",
                "Fehler beim Löschen: " +
                  (await d.text().catch(() => d.status)),
                "error",
              ),
              (t.textContent = "Konto endgültig löschen"),
              void (t.disabled = !1)
            );
      ...
      document
        ...
        .addEventListener("click", async () => {
          ...
          const r = document.getElementById("changePwBtn");
          ((r.textContent = "Wird überprüft…"), (r.disabled = !0));
          const { error: s } = await e.auth.signInWithPassword({
            email: a,
            password: t,
          ...
          if (s)
            return (
              c("pwMsg", "Das aktuelle Passwort ist falsch.", "error"),
              (r.textContent = "Passwort ändern"),
              void (r.disabled = !1)
            );
          r.textContent = "Wird gespeichert…";
          const { error: o } = await e.auth.updateUser({ password: n });
          ((r.textContent = "Passwort ändern"),
            (r.disabled = !1),
            o
              ? c("pwMsg", "Fehler: " + o.message, "error")
              : (c("pwMsg", "Passwort wurde erfolgreich geändert.", "success"),
      ...
      document
        ...
        .addEventListener("click", async () => {
          ...
          ((n.textContent = "Wird gesendet…"), (n.disabled = !0));
          const { error: a } = await e.auth.resetPasswordForEmail(t, {
            redirectTo:
              "https://safenet-security.ch/de/pages/reset-password.html",
          });
          a
            ? (c("forgotErrorSettings", "Fehler: " + a.message, "error"),
              (n.textContent = "Link senden"),
              (n.disabled = !1))
            : ((document.getElementById("forgotSuccessSettings").style.display =
                "block"),
              (n.textContent = "Gesendet!"));
        }),
      document
