...
export default async function handler(e) {
  ...
  const s = new URL(e.url),
    o = s.searchParams.get("id"),
    n = s.searchParams.get("user_id"),
    r = {
      Authorization: t,
  ...
  let a = `${SUPABASE_URL}/rest/v1/passwords`;
  "GET" === e.method
    ? (a += "?select=*&order=created_at.desc")
    : ("PATCH" !== e.method && "DELETE" !== e.method) || !o
      ? "DELETE" === e.method &&
        n &&
        (a += `?user_id=eq.${encodeURIComponent(n)}`)
      : (a += `?id=eq.${encodeURIComponent(o)}`);
  const i =
    "POST" === e.method || "PATCH" === e.method ? await e.text() : void 0;
  try {
    const t = await fetch(a, { method: e.method, headers: r, body: i }),
      s = await t.text();
